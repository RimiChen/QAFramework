...
def initial(data_dir, model_file):
    data_dir       = data_dir
...
def load_model():
    ...
    print(test_var)
    if reversed_dict is None or memory is None or \
           model is None or loss is None or general_config is None:
        print("Loading model from file %s ..." % model_file)
        with gzip.open(model_file, "rb") as f:
            reversed_dict, memory, model, loss, general_config = pickle.load(f)
...
if __name__ == "__main__":
    ...
    initial(args.data_dir, args.model_file)
    load_model()
